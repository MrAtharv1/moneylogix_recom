...
def get_option_chain(symbol: str) -> tuple[dict, str]:
    """
    ...
    try:
        ...
        option_chain_cache.set(symbol, data)
    ...
    # Tier 2: Cache
    cached = option_chain_cache.get(symbol)
    if cached:
    ...
    # Tier 3: DB snapshot
    try:
        # Import lazily to avoid circular imports if database imports fallback somewhere
        from database import get_last_option_data 
        snapshot = get_last_option_data(symbol)
        if snapshot:
